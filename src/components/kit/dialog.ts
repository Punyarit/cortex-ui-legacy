...
export class Dialog extends LitElement {
  ...
  connectedCallback() {
    ...
    wrapper?.addEventListener('click', event => {
      (event.target as HTMLElement).id === 'dialog-wrapper' && this.disableBackdrop === false && this.closeDialog();
    });
  }
  ...
  handleScroll(dialog: HTMLElement, hasVerticalScrollbar?: boolean): void {
    if (!dialog) return;
